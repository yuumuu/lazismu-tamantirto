        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="max-w-3xl space-y-6">
                ...
                <h1 class="text-4xl md:text-6xl font-black text-zinc-900 dark:text-white tracking-tight leading-tight">
                    Berita & Kabar Terbaru {{ setting('site_name', 'Lazismu') }} {{ setting('site_tagline', 'Tamantirto') }}
                </h1>
    ...
    <section class="py-12 bg-white dark:bg-zinc-950 min-h-screen" x-data="{
        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row gap-6 items-center justify-between mb-16">
                <div class="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 w-full lg:w-auto no-scrollbar">
                    <button
                        ...
                        :class="selectedCategory === 'all' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-zinc-50 dark:bg-white/5 text-zinc-500 border border-zinc-100 dark:border-white/5'"
                    ...
                    <template x-for="category in categories" :key="category.id">
                        <button
                            ...
                            :class="selectedCategory === category.id ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-zinc-50 dark:bg-white/5 text-zinc-500 border border-zinc-100 dark:border-white/5'"
                    ...
                    <div>
                        <article class="flex flex-col bg-white dark:bg-zinc-900 rounded-[32px] overflow-hidden border border-zinc-200 dark:border-white/5 shadow-sm hover:shadow-xl transition-all group">
                            <div class="relative aspect-video overflow-hidden">
                                <img :src="post.featured_image ? '/storage/' + post.featured_image : 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=2070&auto=format&fit=crop'" class="size-full object-cover group-hover:scale-110 transition-transform duration-700">
                                <div class="absolute top-6 left-6">
                                    <span class="px-4 py-1.5 rounded-full bg-primary/90 text-white text-[10px] font-black uppercase tracking-widest backdrop-blur-md shadow-lg" x-text="post.category?.name || 'Update'">
                                    </span>
                                </div>
                            </div>
                                ...
                                <div class="space-y-4">
                                    <div class="flex items-center gap-4 text-xs font-bold text-zinc-400">
                                        <span class="flex items-center gap-1.5">
                                            ...
                                            <span x-text="new Date(post.published_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })"></span>
                                        </span>
                                        <span class="flex items-center gap-1.5">
            ...
            <div class="mt-20 text-center">
                <p class="text-sm text-zinc-400 italic">Menampilkan <span x-text="filteredPosts.length"></span> dari {{ $posts->count() }} berita</p>
            </div>
        </div>
    </section>
</x-layouts.guest>
