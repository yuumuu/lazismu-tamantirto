<x-layouts.guest title="Struktur Organisasi">
    <!-- Page Header -->
    <header class="py-20 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-white/5">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="max-w-3xl space-y-6 text-center mx-auto">
                ...
                <h1 class="text-4xl md:text-5xl font-black text-zinc-900 dark:text-white tracking-tight">Keluarga Besar
                    {{ setting('site_name', 'Lazismu') }} {{ setting('site_tagline', 'Tamantirto') }}</h1>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 md:gap-12">
                ...
                @foreach ($members as $member)
                    <div class="flex flex-col items-center text-center space-y-6 group">
                        <div
                            class="relative w-full aspect-[4/5] rounded-[48px] overflow-hidden shadow-2xl border-4 border-zinc-50 dark:border-zinc-900 group-hover:border-primary/20 transition-all">
                            <img src="{{ $member->photo_url }}"
                                class="size-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-105 group-hover:scale-100"
                                alt="{{ $member->name }}">
                            <div
                        ...
                        <div class="space-y-1">
                            <h4
                                class="text-xl font-black text-zinc-900 dark:text-white tracking-tight group-hover:text-primary transition-colors">
                                {{ $member->name }}</h4>
                            <p class="text-xs font-black text-zinc-400 uppercase">
                                {{ $member->position->name ?? 'Staf' }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
