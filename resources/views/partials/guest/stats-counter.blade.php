@php
    $totalMuzakki = \App\Models\Muzakki::active()->count();
    $totalDana = \App\Models\Donation::verified()->sum('amount');
    $activePrograms = \App\Models\Campaign::active()->count();
@endphp
        ...
        <div class="bg-primary/5 dark:bg-primary/10 rounded-[48px] p-8 md:p-16 border border-primary/10 dark:border-primary/20 relative overflow-hidden group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
                <div class="flex flex-col items-center text-center space-y-4">
                    <span class="text-4xl md:text-5xl font-black text-primary tracking-tighter">{{ $totalMuzakki > 100 ? number_format($totalMuzakki / 1000, 1) . 'K+' : $totalMuzakki }}</span>
                    <div class="space-y-1">
                ...
                <div class="flex flex-col items-center text-center space-y-4 border-y md:border-y-0 md:border-x border-primary/20 py-12 md:py-0">
                    <span class="text-4xl md:text-5xl font-black text-primary tracking-tighter">{{ format_rupiah_short($totalDana) }}</span>
                    <div class="space-y-1">
                ...
                <div class="flex flex-col items-center text-center space-y-4">
                    <span class="text-4xl md:text-5xl font-black text-primary tracking-tighter">{{ $activePrograms }}</span>
                    <div class="space-y-1">
