...
<div x-cloak x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-[100] sm:hidden"
    style="display: none;">
    ...
    <div x-transition:enter="transition ease-out duration-300" x-transition:enter-start="-translate-x-full"
        ...
        <div class="p-6 flex items-center justify-between border-b border-zinc-100 dark:border-white/5">
            <div class="flex items-center gap-3">
                <div
                    class="flex aspect-square size-9 items-center justify-center rounded-xl bg-transparent text-white shadow-none">
                    <x-app-logo-icon class="size-6 fill-current" />
                </div>
                <div class="flex flex-col leading-tight">
                    <span
                        class="text-zinc-900 dark:text-white font-black text-lg tracking-tighter uppercase">{{ setting('site_name', 'Lazismu') }}</span>
                    <span
                        class="text-primary font-bold text-[9px] uppercase">{{ setting('site_tagline', 'Tamantirto') }}</span>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-2">
            @foreach ([['route' => 'guest.home', 'label' => 'Beranda', 'icon' => 'home'], ['route' => 'guest.campaigns.index', 'label' => 'Donasi', 'icon' => 'heart'], ['route' => 'guest.posts.index', 'label' => 'Berita', 'icon' => 'newspaper'], ['route' => 'guest.about', 'label' => 'Tentang Kami', 'icon' => 'book-open'], ['route' => 'guest.contact', 'label' => 'Kontak', 'icon' => 'phone'], ['route' => 'guest.calculator', 'label' => 'Kalkulator Zakat', 'icon' => 'calculator']] as $nav)
                <a href="{{ guest_route($nav['route']) }}" @click="open = false"
                    ...
                    <flux:icon name="{{ $nav['icon'] }}"
                        class="size-6 {{ request()->routeIs($nav['route']) || (request()->route('masjid_slug') && request()->routeIs('guest.tenant.' . str_replace('guest.', '', $nav['route']))) ? 'text-primary' : 'text-zinc-400' }}" />
                    {{ $nav['label'] }}
                </a>
            @endforeach
        </div>
        ...
        <div class="p-6 border-t border-zinc-100 dark:border-white/5 space-y-4">
            ...
            <flux:button href="{{ route('guest.donate.form') }}" variant="primary"
                class="w-full font-bold py-4 rounded-2xl shadow-lg shadow-primary/20">
                Donasi Sekarang
            </flux:button>
        </div>
    </div>
