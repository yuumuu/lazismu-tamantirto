{{-- Banners passed from controller --}}
<section x-data="{
    activeSlide: 0,
    ...
    class="relative w-full h-[600px] md:h-[700px] lg:h-[800px] overflow-hidden bg-zinc-900 border-b border-white/5">
        ...
        @foreach ($banners as $index => $banner)
            <div x-show="activeSlide === {{ $index }}" x-transition:enter="transition ease-out duration-1000"
                ...
                <div class="absolute inset-0">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-r from-zinc-950/90 via-zinc-950/50 to-transparent">
                    </div>
                    <div class="absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-zinc-950/80 to-transparent">
                    </div>
                </div>
                ...
                <div class="relative h-full mx-auto max-w-7xl px-6 lg:px-8 flex flex-col justify-center">
                    <div class="max-w-2xl space-y-8">
                        <div class="space-y-4">
                            <h2
                                class="inline-block px-4 py-1.5 rounded-full bg-primary/20 border border-primary/30 text-primary text-xs font-black uppercase animate-fade-in">
                                {{ setting('site_name', 'Lazismu') }} {{ setting('site_tagline', 'Tamantirto') }}
                            </h2>
                            <h1
                                class="text-4xl md:text-6xl lg:text-7xl font-black text-white leading-[1.1] tracking-tight">
                                {{ $banner->title }}
                            </h1>
                            <p class="text-lg md:text-xl text-zinc-300 max-w-xl font-medium">
                                {{ $banner->subtitle }}
                            </p>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-4">
                            @if ($banner->button_link)
                                <flux:button href="{{ $banner->button_link }}" variant="primary"
                                    class="px-8 font-black uppercase tracking-wider h-14 rounded-2xl shadow-2xl shadow-primary/40 group">
                                    {{ $banner->button_text ?? 'Donasi Sekarang' }}
                                    <flux:icon.arrow-right
                                        class="size-5 ms-2 group-hover:translate-x-1 transition-transform" />
                                </flux:button>
                            @endif
                        </div>
                    </div>
